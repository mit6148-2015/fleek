...
def readPagesUSAMO(year):
	...
	aimelink = 'http://www.artofproblemsolving.com/Forum/resources.php?c=182&cid=176&year='+str(year)+'&sid=ebdeeb3829122e94d65c321e5afb4f0d'
	...
	for tb in soup.find_all("span"):
		...
		if tb.get('class') != None and tb.get('class')[0] == "res_gen":
			...
			print(year)
			print (probNum)
			data = {"contest_name":"USAJMO","year":year, "problem_number":probNum, "problem_statement":tb.get_text().strip(), "source_name":"Art of Problem Solving", "source_link": aimelink}
			#print(data)
			probNum = probNum + 1
			currind = currind + 1
